...
var odontogram_edit = function () {
	...
	for (var i = 0; i < data.length; i++) {
		...
		$.ajax({
			...
			url:
				link +
				`pasien_informasi/getTemp/dental${data[i].title}/${id_pasien}/${tgl_pemeriksaan}`,
			dataType: "json",
			success: (hasil) => {
				// console.log(hasil);
				...
				if (hasil.length > 0) {
					...
					for (var i = 0; i < hasil.length; i++) {
						...
						var textNode = document.createTextNode(hasil[i].ket_pemeriksaan);
						newText.appendChild(textNode);
						document
							.getElementById("detailodon" + hasil[i].idSvg)
							.appendChild(newText);
						ysekarang += 6;
					}
				}
