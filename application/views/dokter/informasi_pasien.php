    ...
    <div class="col-lg-9">
      <div class="panel panel-default">
        <div class="panel-body">
          ...
          <table id="all_data_json" data-toggle="table" data-url="<?php echo base_url('doctor/get_informasi_pasien'); ?>" data-show-refresh="false" data-show-toggle="false" data-show-columns="false" data-search="false" data-select-item-name="toolbar1" data-pagination="true" data-sort-name="nama" data-sort-order="asc" data-toolbar="#toolbar">
            <thead>
              <tr>
...
<script>
  ...
  function actionPasien(value, row, index){
    return '<a href="'+base_url+'doctor/detail_informasi_pasien/' +row.id_pasien +'/' + row.id_booking + '/' + row.id_rekam_medis+'.html" class="btn btn-anim" style="height: 35px; width: 70px; background-color: #f40049; color: white; border-radius: 5px"><span> Lihat</span></a>';
  }
  ...
  function cari_data2(){
			...
			$('#all_data_json').bootstrapTable('refresh', {url: '<?php echo base_url()?>Doctor/get_informasi_pasien2/' +  (cari_nama != ""? cari_nama : '0') +'/'+(cari_tgl_lahir != ""? cari_tgl_lahir : '0') +'/'+ (cari_rekam_medis != ""? cari_rekam_medis : '0') });
	}
